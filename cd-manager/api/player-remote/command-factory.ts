...
const getDigitCommandsSequenceForNumber = (
  number: number,
  delayAfterMs: number,
): IrCommandSequenceItem[] => {
...
export const getPlayTrackOrder = (
  trackNumber: number,
): IrCommandSequenceItem[] => {
  const sequence: IrCommandSequenceItem[] = [];
  sequence.push({ command: PlayerCommand.TrackSelect, delayAfterMs: 100 });
  sequence.push(...getDigitCommandsSequenceForNumber(trackNumber, 100));
  sequence.push({ command: PlayerCommand.Play, delayAfterMs: 100 });
  return sequence;
};

export const getSelectDiscOrder = (
  slotNumber: number,
): IrCommandSequenceItem[] => {
  const sequence: IrCommandSequenceItem[] = [];
  ...
  sequence.push({ command: PlayerCommand.Play, delayAfterMs: 8000 });
  return sequence;
};

export const getPlayDiscOrder = (
  slotNumber: number,
): IrCommandSequenceItem[] => {
  const sequence: IrCommandSequenceItem[] = [];
...
export const getPlayTrackOnDiskOrder = (
  slotNumber: number,
  trackNumber: number,
): IrCommandSequenceItem[] => {
  const sequence: IrCommandSequenceItem[] = [];
