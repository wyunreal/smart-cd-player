...
describe("RateLimiter", () => {
  ...
  it("blocks requests over the limit", async () => {
    ...
    expect(limiter.isRateLimited("10.0.0.1")).toBe(false); // 1
    expect(limiter.isRateLimited("10.0.0.1")).toBe(false); // 2
    expect(limiter.isRateLimited("10.0.0.1")).toBe(false); // 3
    expect(limiter.isRateLimited("10.0.0.1")).toBe(true);  // 4 → blocked
  });
  ...
  it("tracks different IPs independently", async () => {
    ...
    expect(limiter.isRateLimited("1.1.1.1")).toBe(false);
    expect(limiter.isRateLimited("1.1.1.1")).toBe(true); // blocked

    expect(limiter.isRateLimited("2.2.2.2")).toBe(false); // different IP, not blocked
  });
  ...
  it("allows requests again after the window expires", async () => {
    ...
    expect(limiter.isRateLimited("3.3.3.3")).toBe(false);
    expect(limiter.isRateLimited("3.3.3.3")).toBe(false);
    expect(limiter.isRateLimited("3.3.3.3")).toBe(true); // blocked

    // Wait for window to expire
    await new Promise((resolve) => setTimeout(resolve, 150));

    expect(limiter.isRateLimited("3.3.3.3")).toBe(false); // allowed again
  });
});
